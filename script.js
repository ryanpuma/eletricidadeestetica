...
// Função para obter o código de idioma do navegador
function getLanguage() {
    // Obtém o idioma preferencial do navegador
    var userLang = navigator.language || navigator.userLanguage;

    // Retorna o código de idioma (por exemplo, "en" para inglês, "pt" para português)
    return userLang.substr(0, 2); // Apenas os dois primeiros caracteres (ex: "en-US" -> "en")
}
...
function redirectByLanguage() {
    ...
    // Se o idioma do navegador for inglês, redirecione para a versão em inglês
    if (lang === "en") {
        window.location.href = "/en" + window.location.pathname; // Redireciona para a pasta 'en'
    }
    // Caso contrário, mantém na versão padrão em português (não precisa de redirecionamento)
}

// Chama a função de redirecionamento quando a página terminar de carregar
document.addEventListener("DOMContentLoaded", function() {
    redirectByLanguage();
});
